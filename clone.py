...
class Clone:
    ...
    def findMove(self, coords: tuple, grid):
        """
        coords is a tuple representing the location of the player
        grid class object for node creation
        """
        ma = grid.returnGridRepr()
        # Find the closest path from start(@) to end($)
        path = astar_search(ma, (self.x, self.y), coords)
        print()
        print(path)
        print()
...
def astar_search(map, start, end):
    ...
    start_node = _Node(start, None)
    goal_node = _Node(end, None)
    # Add the start node
    ...
    while len(open) > 0:
        ...
        for next in neighbors:
            ...
            neighbor = _Node(next, current_node)
